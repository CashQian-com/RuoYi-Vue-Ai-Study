...
"""
Anthropic Claude API集成工具 - Claude Code Hooks系统组件

该脚本提供Anthropic Claude API的集成功能，支持基本的LLM提示和完成消息生成。
使用最快的Claude模型提供高效的AI生成服务。

功能特性：
- 优先使用claude-3-5-haiku-20241022模型（最快速）
- 支持工程师姓名个性化的完成消息
- 自动清理和格式化响应内容
- 健壮的错误处理和失败备用
- 命令行界面支持
...
使用方法：
    python anth.py "your prompt here"     # 基本LLM提示
    python anth.py --completion           # 生成完成消息
...
def prompt_llm(prompt_text):
    ...
    功能特性：
    - 使用claude-3-5-haiku-20241022模型（最快速）
    ...
    try:
        ...
        message = client.messages.create(
            model="claude-3-5-haiku-20241022",  # 最快的Anthropic模型
            max_tokens=100,
            temperature=0.7,
...
def main():
    ...
    支持的命令：
    - anth.py "prompt text"     # 直接LLM提示
    - anth.py --completion      # 生成完成消息
    - anth.py                   # 显示使用说明
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--completion":
            ...
            if response:
                ...
                print("调用Anthropic API时出错")
    else:
        print("使用方法: ./anth.py '您的提示内容' 或 ./anth.py --completion")
